...
//
// The functions and variables in this module must be provided by a slave implementation.
...
#ifndef __OPDI_DEVICE_H
#define __OPDI_DEVICE_H
...
extern const char *opdi_slave_name;

/** A pointer to a buffer of at least length MASTER_NAME_LENGTH that accepts the connecting master's name. */
extern char opdi_master_name[];

/** The encoding that should be used. May be empty or NULL. */
...
/** A comma-separated list of protocols supported by this device. */
...
/** Flag combination that is valid for this device.
*/
extern uint16_t opdi_device_flags;

/** Used for debug messages. All sent or received messages are passed to this function.
*   This function must be provided by a slave implementation. It may choose to return
...
*   Must return OPDI_STATUS_OK if everything is ok.
*/
extern uint8_t opdi_get_dial_port_state(opdi_Port *port, int32_t *position);

/** Sets the position of a dial port. 
*   This function is typically provided by a configuration.
*   Must return OPDI_STATUS_OK if everything is ok.
*/
extern uint8_t opdi_set_dial_port_position(opdi_Port *port, int32_t position);
