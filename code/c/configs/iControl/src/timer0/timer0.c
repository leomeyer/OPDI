...
void timer0_init(void)
{
	...
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {	
		...
		TC_SetPeriod(&TIMER, (uint16_t)(F_OSC / 256 * 1e-3 - 0.5));         // Set period (Example for 32 MHz: 125 ticks = 1ms)
		TC0_SetOverflowIntLevel(&TIMER, TC_OVFINTLVL_MED_gc);  // Set Interrupt on Overflow
		TC0_ConfigClockSource(&TIMER, TC_CLKSEL_DIV256_gc);    // Set clock and prescaler (8us per tick)
