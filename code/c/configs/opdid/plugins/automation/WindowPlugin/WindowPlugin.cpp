...
#define CHECK_CLOSE															\
	...
	if (this->currentState == CLOSING) {
		...
		// closing time twice up without detecting sensor? error condition
		if (opdi_get_time_ms() - this->openTimer > 2 * this->openingTime) {
			// stop motor
			this->setMotorOff();
...
void WindowPlugin::setupPlugin(AbstractOPDID *abstractOPDID, std::string node, Poco::Util::AbstractConfiguration *nodeConfig) {
	...
	if (portType == "SelectPort") {
		...
		port->sensor = abstractOPDID->getConfigString(nodeConfig, "Sensor", "", true);
