...
import org.ospdi.opdi.wizdroyd.bluetooth.AddBluetoothDevice;
import org.ospdi.opdi.wizdroyd.bluetooth.BluetoothDevice;
import org.ospdi.opdi.wizdroyd.bluetooth.EditBluetoothDevice;
import org.ospdi.opdi.wizdroyd.gui.LoggingActivity;
import org.ospdi.opdi.wizdroyd.ports.ShowDevicePorts;
import org.ospdi.opdi.wizdroyd.tcpip.AddTCPIPDevice;
import org.ospdi.opdi.wizdroyd.tcpip.EditTCPIPDevice;
import org.ospdi.opdi.wizdroyd.tcpip.TCPIPDevice;
...
/** This is the main activity for the Wizdroyd client.
 * 
 * @author Leo
 *
 */
public class Wizdroyd extends LoggingActivity implements DeviceManager.IDeviceStatusListener {
	
	/** This client's name as it is sent to slave devices. */
	public static final String MASTER_NAME = "Wizdroyd";
	...
	public static Wizdroyd instance;
	...
	private WizdroydDevice selectedDevice;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        org.ospdi.opdi.utils.ResourceFactory.instance = new org.ospdi.opdi.wizdroyd.utils.ResourceFactory(this);
        ...
        lvDevices.setOnItemClickListener(new OnItemClickListener() {
        	...
        	public void onItemClick(AdapterView<?> av, View view, int position, long id) {
        		...
        		if (deviceManager.getDevices().get(position) == null)
    				...
    				final WizdroydDevice selDevice = deviceManager.getDevices().get(position);
	...
	private void connectToDevice(WizdroydDevice selDevice, boolean noConfirmation) {
		...
		if (message == null) {
        	Wizdroyd.this.selectedDevice = selDevice;
        	Wizdroyd.this.connectSelectedDevice();
        }
		else {
			// show message dialog
	        new AlertDialog.Builder(Wizdroyd.this)
	...
	protected void showDeviceCapabilities(WizdroydDevice device) {
	...
	private void openDeviceMenu(WizdroydDevice iDevice) {
...
/*		
		...
		for (WizdroydDevice device: deviceManager.getDevices())
...
*/
	...
	private void scanQRCode() {
        ...
        IntentIntegrator.initiateScan(Wizdroyd.this, R.layout.capture,
        ...
        switch (requestCode) {
            ...
            case ADD_BLUETOOTH_DEVICE: {
                if (resultCode == Activity.RESULT_CANCELED)
                	return;
            	String ser = data.getStringExtra(Wizdroyd.DEVICE_SERIALIZATION);
            ...
            case EDIT_BLUETOOTH_DEVICE: {
                if (resultCode == Activity.RESULT_CANCELED)
                	return;
            	String ser = data.getStringExtra(Wizdroyd.DEVICE_SERIALIZATION);
            	String prevSer = data.getStringExtra(Wizdroyd.DEVICE_PREVIOUS_SERIALIZATION);
				BluetoothDevice device = new BluetoothDevice(ser);
	...
	public synchronized boolean addDevice(WizdroydDevice device) {
		
		// check whether this device already exists
		for (WizdroydDevice aDevice: deviceManager.getDevices()) {
			if (aDevice == null)
				continue;
	...
	private synchronized void editDevice(WizdroydDevice device) {
	...
	private synchronized void removeDevice(WizdroydDevice device) {
	...
	@Override
	public synchronized boolean getCredentials(WizdroydDevice device, String[] namePassword, Boolean[] save) {
		...
		final Thread runnable = new Thread() {
			...
			public void run() {
				...
				final Dialog dl = new Dialog(Wizdroyd.this);
	 ...
	 * @return
	 */
	public synchronized WizdroydDevice findDeviceByAddress(String devAddress) {
	...
	public void notifyDevicesChanged(final String message, final boolean error) {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				deviceStatusChanged();
				if (message != null) {
					Toast.makeText(Wizdroyd.this, message, (error ? Toast.LENGTH_LONG : Toast.LENGTH_SHORT)).show();
	...
	@Override
	public void receivedDebug(WizdroydDevice device, String message) {
		if (isFinishing()) return;
		...
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(Wizdroyd.this, msg, Toast.LENGTH_SHORT).show();
				addLogMessage(msg);
			}
		});
	}
	...
	@Override
	public void receivedError(WizdroydDevice device, String text) {
